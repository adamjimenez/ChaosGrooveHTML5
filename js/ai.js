...
function AI_analyse_board() {
	var w, w2, num, strength, risk;
	var text;
	...
	for (w = 0; w < MAX_WIZARDS; w++) {
		...
		//console.log("w: %d, threat: %d", w, ai[w].threat);
	}
	...
	for (w = 0; w < MAX_WIZARDS; w++) {
		...
		if (game.debug_to_chat) {
			//console.log("w: %d, strength: %d", w, ai[w].strength);
...
function AI_select_spell() {
	var x, y, a, s, r, sp, score, best_score, value, best_value, best_x, best_y, best_spell, extra_combat, extra_defence;
	var illusion, best_illusion;
	var text;
	...
	//console.log("AI Wizard : %d - selecting spell..", game.current_wizard);
	//console.log("");

	// Now find how much our wizard has been 'improved'
	text = sprintf("wizard_%d", game.current_wizard);
...
;
	...
	for (s = 0; s < MAX_WIZ_SPELLS; s++) {
		...
		//console.log("spell: %s", spell[sp].name);
		score = 0;
		illusion = false;
		...
		if (spell[sp].piece_ID != -1) {
			...
			if (spell[sp].enemy || spell[sp].own) {
				for (y = 0; y < board_info.board_height; y++) {
					for (x = 0; x < board_info.board_width; x++) {
						...
						if (is_piece_friendly(x, y, PIECE) && strlen(spell[sp].new_piece_name) > 0) {
							...
							//console.log("x: %d, y: %d, name: %s, value: %d", x, y, board[x][y][PIECE].name, value);
						}
				...
				//console.log("* best_x : %d, best_y: %d, best_value: %d", best_x, best_y, best_value);
		...
		if (spell[sp].range == 0) {
			if (spell[sp].change_shadow_form_to && !board[board_info.selected_x][board_info.selected_y][board_info.selected_layer].shadow_form) {
			...
			// Movement
			if (spell[sp].change_movement_amount_to > board[board_info.selected_x][board_info.selected_y][board_info.selected_layer].movement) {
			...
			if (spell[sp].change_flying_to == 1 && !board[board_info.selected_x][board_info.selected_y][board_info.selected_layer].flying) score += 100;

			// Ranged Combat
			if (spell[sp].change_ranged_combat_attack_to > board[board_info.selected_x][board_info.selected_y][board_info.selected_layer].ranged_combat_attack) {
		...
		if (return_spell_chance(sp) < 100 - (r * 10)) {
			...
			if (spell[sp].can_be_illusion && return_spell_chance(sp) < 100) {
				...
				//if (score > 0) console.log("* name: %s, num: %d, score: %d, a: %d, chance: %d", spell[sp].name, sp, score, a,
				...
				if (IRand(0, 20) < a) {
					...
					//console.log("Too risky!");
				}
			}
		}

		//if (score > 0) console.log("name: %s, num: %d, score: %d", spell[sp].name, sp, score);
	...
	//console.log("best score: %d, best spell: %d, name: %s", best_score, best_spell,
...
function AI_select_spell_square() {
	var x, y, p, best_x, best_y, x1, y1, x2, y2, range;
	var dist, best_dist;
	var defensive;
	var name;
	...
	for (y = 0; y < board_info.board_height; y++) {
		for (x = 0; x < board_info.board_width; x++) {
			...
			if (highlight_board[x][y].type == HIGHLIGHT_CAN_RIDE) {
				...
				if (is_piece_friendly(x, y, PIECE) && strlen(spell[wizard[game.current_wizard].spell].new_piece_name) > 0) {
...
function AI_select_target(tx, ty, range, combine, wiz) {
	...
	var x, y, best_x, best_y, value, best_value, total;
	var dist;
...
function AI_select_ranged_target() {
	// This function is used to find a target for movement and spell casting.
	...
	var x, y, tx, ty, best_x, best_y, value, best_value, total;
...
function AI_rate_spell_piece(sp, risk) {
	var p, wx, wy, strength, final_value, def, add, value = 0;
	...
	//console.log("name: %s - rated: %d - chance: %d, Final Value: %d", piece[p].name, value, spell[sp].chance, final_value);

	return final_value;
}

function AI_rate_piece(x, y, tx, ty, layer, wiz) {
	// This rates a piece at x, y based on it's stats and the distances from both
	// the current wizard, and from tx, ty.

	// We should a hardcoding preferences, to make the piece data user-definable.

	var total, range_from_wizard, range_from_selected, old_range_from_wizard, old_range_from_selected;
	var value, w, add;
	...
	// Add distances.
	value = value + range_from_selected + range_from_wizard;
...
function AI_move_piece() {
	// This moves an individual piece until it is finished.

	var x, y, best_x, best_y, x1, y1, x2, y2, range, total;
	var dist, best_dist, value, best_value;
	...
	if (game.debug_to_chat) console.log("AI moving piece : %s", board[board_info.selected_x][board_info.selected_y][board_info.selected_layer].name);
	...
	// A Wizard should generally a enemies instead of moving towards them..
	if (board_info.selected_x == wizard[game.current_wizard].x && board_info.selected_y == wizard[game.current_wizard].y) {
		...
		if (ai[game.current_wizard].target_x == -1 || (ai[game.current_wizard].target_x == board_info.selected_x && ai[game.current_wizard].target_y == board_info.selected_y) && board[board_info.selected_x][board_info.selected_y][MOUNTED].gfx != -1) {
			// Our wizard isn't moving, but is mounted.. so maybe can shoot from mount?
			board_info.selected_layer = MOUNTED;
		}
	}

	if (ai[game.current_wizard].target_x == -1 || (ai[game.current_wizard].target_x == board_info.selected_x && ai[game.current_wizard].target_y == board_info.selected_y)) {
		// If we have no targets left or we need to stand still, then end move.
		//end_move_piece();
		if (game.debug_to_chat) console.log("target x: %d, target y: %d", ai[game.current_wizard].target_x, ai[game.current_wizard].target_y);
		...
		if (game.debug_to_chat) console.log("Ending piece move");
	}
	...
	/*
 console.log("name: %s, x: %d, y: %d, layer: %d, has_moved: %d, engaged: %d, shot: %d", 
 ...
 */
	...
	//console.log("shooting: %s, x: %d, y: %d, layer: %d", board[board_info.selected_x][board_info.selected_y][board_info.selected_layer].name,
	//board_info.selected_x, board_info.selected_y, board_info.selected_layer);
...
function AI_do_wizard_movement() {
	var x, y, wx, wy, value, best_value, best_x, best_y;
	var range;
	var enemy, ride;
	var name;
	...
	//console.log("wizard: %d", game.current_wizard);
	...
	for (y = 0; y < board_info.board_height; y++) {
		for (x = 0; x < board_info.board_width; x++) {
			...
			if (highlight_board[x][y].type == HIGHLIGHT_CAN_ATTACK && !ride && !board[board_info.selected_x][board_info.selected_y][board_info.selected_layer].shadow_form) {
				// Unless our wizard can outrun or outfly this enemy, then attack! Except if we have shadow form..
				range = workout_range(x, y, wx, wy);
				//console.log("range: %f, movement: %d, x: %d, y: %d, wx: %d, wy: %d", range, board[wx][wy][PIECE].movement, x, y, wx, wy);

				// Is piece too close to a?
			...
			if ((x == 0 || y == 0 || x == board_info.board_width - 1 || y == board_info.board_height - 1) && ai[game.current_wizard].strength >= 0) value -= 500;

			if ((x == 1 || y == 1 || x == board_info.board_width - 2 || y == board_info.board_height - 2) && ai[game.current_wizard].strength >= 1000) value -= 250;
	...
	//console.log("wizard: %d at x: %d, y: %d, enemy: %d, ride: %d", game.current_wizard, wx, wy, enemy,ride);
...
function find_threat_value_to_wizard(w) {
	var x, y;
